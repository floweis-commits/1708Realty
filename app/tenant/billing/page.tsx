  ...
  return (
    <div className="space-y-12">
      ...
      {searchParams.success && (
        <div className="shell p-6 body-sm text-ink">Payment confirmed. Thank you.</div>
      )}
      ...
      <div className="shell p-12 space-y-6">
        ...
        {payment?.current_period_end && (
          <div className="body-sm">
            Next payment: {new Date(payment.current_period_end).toLocaleDateString()}
          </div>
        )}
        {payment?.amount && (
          <div className="body-sm">Amount: ${(payment.amount / 100).toFixed(2)} / month</div>
        )}
      </div>
      ...
      <div className="flex gap-8">
        {!payment || payment.status === "canceled" || payment.status === "incomplete" ? (
          ...
          <form action="/api/portal" method="post">
            <button className="btn-secondary">Manage Card</button>
          </form>
        )}
      </div>
    </div>
