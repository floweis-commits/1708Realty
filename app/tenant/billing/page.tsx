    ...
    <div className="space-y-12">
      ...
      <div className="shell p-10 space-y-8">
        <div className="flex items-start justify-between">
          ...
          {latest && (
            <span
              className={`label-md px-3 py-1.5 ${statusColor[latest.status] ?? "text-secondary bg-[#D6D3D1]"}`}
            >
          ...
          <div className="shell">
            ...
            <ul>
              {payments.map((p) => (
                <li
                  key={p.id}
                  className="flex items-center gap-8 px-8 py-5 border-b border-line last:border-0"
                >
                  ...
                  <span className="w-28 text-right">
                    <span
                      className={`label-md px-2 py-1 ${statusColor[p.status] ?? "text-secondary bg-[#D6D3D1]"}`}
                    >
