...
export default async function TenantDashboard() {
  ...
  const { data: orders } = await supabase.from("work_orders").select("id,status").eq("tenant_id", user!.id);
  const open = orders?.filter((o) => o.status !== "Completed" && o.status !== "Closed").length ?? 0;
  const { data: payment } = await supabase.from("payments").select("status,current_period_end").eq("tenant_id", user!.id).order("created_at", { ascending: false }).limit(1).maybeSingle();
  return (
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Link href="/tenant/billing" className="shell p-12 block hover:bg-stone-50 transition-colors">
          ...
          <div className="text-4xl text-ink font-medium tracking-tight">{payment?.status === "active" ? "Active" : payment?.status ?? "Not started"}</div>
          {payment?.current_period_end && <div className="body-sm mt-4">Next: {new Date(payment.current_period_end).toLocaleDateString()}</div>}
        </Link>
