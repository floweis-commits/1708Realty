    ...
    <div className="space-y-12">
      ...
      <div className="shell">
        {(!payments || payments.length === 0) ? (
          ...
          <ul>{(payments as PaymentRow[]).map((p) => (
            <li key={p.id} className="flex items-center gap-8 px-8 py-6 border-b border-line last:border-0">
              ...
              <span className="label-md text-secondary w-40 text-right">{p.current_period_end ? new Date(p.current_period_end).toLocaleDateString() : "—"}</span>
            </li>
          ))}</ul>
        )}
      </div>
