...
export default function StatusForm({ id, current }: { id: string; current: string }) {
  ...
  return (
    <div className="flex items-center gap-8">
      <select value={status} onChange={(e) => setStatus(e.target.value)} className="input-underline bg-transparent flex-1">
        {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
      </select>
      <button onClick={save} disabled={saving || status === current} className="btn-primary">
